...
# If Doctors didn't show their preference， give them maximum penalty
    def prefs_to_cost(doctors, hospitals, preferences, large_penalty=None):
    ...
    def solve_doctor_hospital(doctors, hospitals, capacities, preferences, large_penalty=None):

        # Preference
        A = prefs_to_cost(doctors, hospitals, preferences, large_penalty=large_penalty)  # (N,K)
        # Set of Capacity
        c = np.array([int(capacities.get(h, 0)) for h in hospitals], dtype=int)  # (K,)

        match = assignment(A, c)
...
# Compute the rate about average rank and percentage for each rank
    def compute_metrics(doctors, preferences, assigned):
...
# ASCII Table to have a clear version for Doctor/ Hospital/ Status/ Rank
    def print_table_ascii(doctors, hospitals, preferences, assigned, eliminated):

        rows = []
        for d in doctors:
            h = assigned.get(d)
            status = "Out" if d in eliminated else "In"
            pref = preferences.get(d, [])
            rank = (pref.index(h) + 1) if (h is not None and h in pref) else None
            rows.append((d, h, status, rank))
        ...
        print("-" * (w_doc + 1 + w_hos + 1 + w_st + 1 + w_rk))
        for d, h, st, rk in rows:
            rk_str = "" if rk is None else str(rk)
            print(f"{str(d):<{w_doc}} {str(h):<{w_hos}} {st:<{w_st}} {rk_str:>{w_rk}}")
    ...
    if __name__ == "__main__":
        ...
        assigned, eliminated, match_matrix = solve_doctor_hospital(
            doctors, hospitals, capacities, preferences
        )
